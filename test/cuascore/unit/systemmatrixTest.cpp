...
TEST(fillMatrixTest, randomValuesSystemmatrixDeprecated) {
  if (mpiSize != 1) {
    return;
  }
  ...
  PETScGrid bndMaskGrid(NX, NY);
  {
    ...
    auto TeffWrite = effTrans.getWriteHandle();
...
TEST(fillMatrixTest, randomValues) {
  ...
  PETScGrid SeGrid(NX, NY);
  PETScGrid effTrans(NX, NY);
  PETScGrid effTransNorth(NX, NY);
  PETScGrid effTransEast(NX, NY);
  PETScGrid effTransSouth(NX, NY);
  PETScGrid effTransWest(NX, NY);
  PETScGrid u_nGrid(NX, NY);
  PETScGrid QGrid(NX, NY);
  PETScGrid dirichValGrid(NX, NY);
  PETScGrid bndMaskGrid(NX, NY);
  PETScGrid topgGrid(NX, NY);
  topgGrid.setZero();
  PETScGrid headGrid(NX, NY);
  headGrid.setConst(100.0);
  {
    auto SeWrite = SeGrid.getWriteHandle();
    auto TeffWrite = effTrans.getWriteHandle();  // write just one and copy later
    auto u_nWrite = u_nGrid.getWriteHandle();
    auto QWrite = QGrid.getWriteHandle();
    auto dirichValWrite = dirichValGrid.getWriteHandle();
    auto bndMaskWrite = bndMaskGrid.getWriteHandle();
  ...
  CUAS::updateInterfaceTransmissivityCDS(effTransEast, effTransWest, effTransNorth, effTransSouth, bndMaskGrid,
                                         topgGrid, headGrid, effTrans);

  CUAS::systemmatrix(matA, gridB, SeGrid, effTransEast, effTransWest, effTransNorth, effTransSouth, dx, dtsecs, theta,
                     u_nGrid, forcing->getCurrent(0), dirichValGrid, bndMaskGrid, globalIndices);
...
TEST(fillMatrixTest, randomValues3x3) {
  ...
  PETScGrid effTrans(gridedge, gridedge);
  PETScGrid effTransNorth(gridedge, gridedge);
  PETScGrid effTransEast(gridedge, gridedge);
  PETScGrid effTransSouth(gridedge, gridedge);
  PETScGrid effTransWest(gridedge, gridedge);
  ...
  PETScGrid topgGrid(gridedge, gridedge);
  topgGrid.setZero();
  PETScGrid headGrid(gridedge, gridedge);
  headGrid.setConst(100.0);
  {
    ...
    for (int row = 0; row < SeGrid.getLocalNumOfRows(); ++row) {
      for (int col = 0; col < SeGrid.getLocalNumOfCols(); ++col) {
        ...
        TeffWrite(row, col) = TeffPow[cornerY + row][cornerX + col];
