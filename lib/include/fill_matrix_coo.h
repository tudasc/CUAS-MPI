...
PetscScalar hmean(PetscScalar x1, PetscScalar x2) { return 2.0 * x1 * x2 / (x1 + x2 + TINY); }

int m(int i, int j, int Nx) { return j * Nx + i; }

void fill_matrix_coo(PetscMat *A, PetscVec *b, int Nx, int Ny, PetscGrid *S, PetscGrid *T, PetscScalar dx,
                     PetscScalar dt, PetscScalar theta, PetscGrid *u_n, PetscGrid *Q, PetscGrid *dirichlet_values,
