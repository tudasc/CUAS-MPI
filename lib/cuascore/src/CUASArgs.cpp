...
void parseArgs(int argc, char **argv, CUASArgs &args) {
  ...
  options.add_options()("x,Tmax", "Maximum T to be allowed in the evolution.",
      ...
      "basalVelocityIce", "Basal velocity of the ice (m/s)", cxxopts::value<PetscScalar>()->default_value("1e-6"))(
      "cavityBeta", "cavity opening parameter", cxxopts::value<PetscScalar>()->default_value("5e-4"))(
      "initialHead", "Initial value for head. Nzero means that head is set such that effective pressure N is zero.",
      cxxopts::value<std::string>()->default_value("Nzero"))(
      "tempResults", "Save temporary results to this file(s) to later restart from them.",
      cxxopts::value<std::string>()->default_value(""))("version", "Show version information")(
      "seaLevelForcing", "Apply sea level forcing from netcdf scalar time series file.",
      cxxopts::value<std::string>()->default_value(""))("v,verbose", "Verbose output. Disables Progressbar");
