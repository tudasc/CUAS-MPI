...
void gradient2(PetscGrid &h, PetscGrid &gradient, PetscScalar const dx) {
  gradient.setZero();
  auto localh = h.getAsLocal2dArr();
  auto grad = gradient.getAsGlobal2dArr();
  for (int i = 0; i < gradient.getLocalNumOfRows(); ++i) {
    for (int j = 0; j < gradient.getLocalNumOfCols(); ++j) {
      int indexI = i + 1;
      int indexJ = j + 1;
      PetscScalar grad_x = 0.5 * (pow((localh[indexI][indexJ] - localh[indexI - 1][indexJ]) / dx, 2) +
                                  pow((localh[indexI + 1][indexJ] - localh[indexI][indexJ]) / dx, 2));
      PetscScalar grad_y = 0.5 * (pow((localh[indexI][indexJ] - localh[indexI][indexJ - 1]) / dx, 2) +
...
void gradient2_central(PetscGrid &h, PetscGrid &gradient, PetscScalar const dx) {
  ...
  for (int i = 0; i < gradient.getLocalNumOfRows(); ++i) {
    for (int j = 0; j < gradient.getLocalNumOfCols(); ++j) {
      int indexI = i + 1;
      int indexJ = j + 1;
      PetscScalar grad_x = (localh[indexI + 1][indexJ] - localh[indexI - 1][indexJ]) / (2 * dx);
      PetscScalar grad_y = (localh[indexI][indexJ + 1] - localh[indexI][indexJ - 1]) / (2 * dx);
